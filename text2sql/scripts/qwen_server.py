"""
Server for fine-tuned Qwen2.5-Coder-0.5B model.
...
LORA_MODEL = Path(__file__).parent.parent.parent / "qwen0p5b-spider-lora"

def load_qwen_model():
    """Load base Qwen model and apply LoRA adapters."""
    global model, tokenizer
    
    print("="*80)
    print("LOADING FINE-TUNED QWEN MODEL")
    print("="*80)
    print("This will load the base model and apply LoRA adapters.")
    print()
    
    if not LORA_MODEL.exists():
        print(f"❌ ERROR: LoRA model not found at {LORA_MODEL}")
        print("Please run finetune_qwen.py first to train the model!")
        return None, None
    ...
    model = PeftModel.from_pretrained(base_model, str(LORA_MODEL))
...
def generate():
    ...
    try:
        # Qwen model was trained on format: SCHEMA: ... Q: ... SQL: ...
        # Ensure prompt matches this format
...
def tags():
    ...
    return jsonify({
        'models': [{
            ...
            'size': 0,
            'digest': 'qwen-0.5b-spider'
        }]
    })

if __name__ == '__main__':
    print("="*80)
    print("FINE-TUNED QWEN MODEL INFERENCE SERVER")
    print("="*80)
    print("This server exposes the fine-tuned Qwen2.5-Coder-0.5B model via an Ollama-compatible API.")
    print()
    ...
    print("="*80)
    print("URL: http://localhost:11438")
    print("Model name: ollama/qwen-0.5b-spider")
    print("="*80)
    print()
    ...
    try:
        app.run(host='0.0.0.0', port=11438, debug=False)
    except OSError as e:
